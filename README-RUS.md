...
В первую очередь необходимо добавить собственную url-схему и хост. Это необходимо для возврата из браузера обратно в приложение после синхронизации cookie пользователя. Если вы используете *setDisabledOpenLinkAction(true)* в [Plus1Request](https://github.com/WapStart/plus1-android-sdk/blob/master/doc/Plus1Request.md), вы можете пропустить этот раздел (не рекомендуется).
...
Установите *callback url* методом **setCallbackUrl()** согласно вашим настройкам манифеста. Если вы используете *setDisabledOpenLinkAction(true)* в [Plus1Request](https://github.com/WapStart/plus1-android-sdk/blob/master/doc/Plus1Request.md), это делать необязательно (не рекомендуется).
